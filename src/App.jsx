...
// -------------------- Mobile Menu --------------------
const MobileMenu = ({ isNavOpen, setIsNavOpen, navRef, toggleButtonRef, headerHeight }) => {
  ...
  return (
    <div
      ref={navRef}
      className={`md:hidden fixed left-0 w-full bg-gray-950/80 backdrop-blur-lg transition-all duration-300 ease-in-out transform ${isNavOpen ? 'translate-x-0 opacity-100 z-40' : '-translate-x-full opacity-0 -z-10'}`}
...
const Header = ({ isNavOpen, setIsNavOpen }) => {
  ...
  return (
    <header ref={headerRef} className="fixed top-0 z-50 bg-gray-950 bg-opacity-70 backdrop-blur-md p-4 w-full" role="banner">
      ...
      <MobileMenu isNavOpen={isNavOpen} setIsNavOpen={setIsNavOpen} navRef={mobileNavRef} toggleButtonRef={toggleButtonRef} headerHeight={headerHeight} />
    </header>
  );
};
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {agencyData.pricing.map((plan) => (
          ...
          >
            ...
            <a
              href="#contact"
              className="mt-8 block relative overflow-hidden px-5 py-3 rounded-full font-bold text-white bg-purple-600 transition-all duration-200 hover:shadow-lg hover:shadow-purple-500/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-400 disabled:opacity-50 disabled:cursor-not-allowed"
            >
